...
content_id: a731312d1971969a29659993277c53eb7cff3215b8caa72489fa9ce11e6708b2
---
...
export default class FilterConcept {
  ...
  async createFilter(): Promise<{ filter: Filter } | { error: string }> {
    ...
    try {
      ...
      // TS18046 Fix: Narrow type of 'e'
      const errorMessage = e instanceof Error ? e.message : String(e);
      return { error: `Failed to create filter: ${errorMessage}` };
    }
  }
  ...
  async addCriteria(
    ...
    try {
      ...
      // TS18046 Fix: Narrow type of 'e'
      const errorMessage = e instanceof Error ? e.message : String(e);
      return { error: `Failed to add criteria: ${errorMessage}` };
    }
  }
  ...
  async search(
    ...
    for (const item of items) {
      ...
      for (const key in criteria) {
        if (Object.prototype.hasOwnProperty.call(criteria, key)) { // Safely check for own properties
  ...
  async clearFilter(
    ...
    try {
      ...
      // TS18046 Fix: Narrow type of 'e'
      const errorMessage = e instanceof Error ? e.message : String(e);
      return { error: `Failed to clear filter: ${errorMessage}` };
    }
  }
}
```
